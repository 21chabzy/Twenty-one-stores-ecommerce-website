...
<?php } ?>
    ...
    <a href="cancelled-orders.php">
        ...
        $status = 'Cancelled'; 
        $cancelledOrdersQuery = mysqli_query($con, "SELECT * FROM Orders WHERE orderStatus='$status'");
        $cancelledOrdersCount = mysqli_num_rows($cancelledOrdersQuery);
        if ($cancelledOrdersCount > 0) {
        ?>
        <b class="label red pull-right"><?php echo htmlentities($cancelledOrdersCount); ?></b>
        <?php } ?>
    </a>
</li>
